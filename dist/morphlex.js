...
function morphNodes(node, ref, idMap) {
    if (isElement(node) && isElement(ref) && node.tagName === ref.tagName) {
        ...
        if (!isInput(node) && !isOption(node) && !isTextArea(node) && node.isEqualNode(ref))
            return;
        else {
            if (node.hasAttributes() || ref.hasAttributes())
                morphAttributes(node, ref);
            if (node.hasChildNodes() || ref.hasChildNodes())
                morphChildNodes(node, ref, idMap);
        }
    }
    else {
        if (node.isEqualNode(ref))
            return;
        else if (isText(node) && isText(ref)) {
            if (node.textContent !== ref.textContent)
                node.textContent = ref.textContent;
        }
        else if (isComment(node) && isComment(ref)) {
            if (node.nodeValue !== ref.nodeValue)
                node.nodeValue = ref.nodeValue;
        }
        else
            node.replaceWith(ref.cloneNode(true));
    }
}
function morphAttributes(elm, ref) {
    ...
    for (const { name } of elm.attributes)
        ref.hasAttribute(name) || elm.removeAttribute(name);
    // Copy attributes from the reference to the element, if they don’t already match.
    for (const { name, value } of ref.attributes)
        elm.getAttribute(name) === value || elm.setAttribute(name, value);
    ...
    if (isInput(elm) && isInput(ref)) {
        if (elm.checked !== ref.checked)
            elm.checked = ref.checked;
        if (elm.disabled !== ref.disabled)
            elm.disabled = ref.disabled;
        if (elm.indeterminate !== ref.indeterminate)
            elm.indeterminate = ref.indeterminate;
        if (elm.type !== "file" && elm.value !== ref.value)
            elm.value = ref.value;
    }
    else if (isOption(elm) && isOption(ref) && elm.selected !== ref.selected)
        elm.selected = ref.selected;
    else if (isTextArea(elm) && isTextArea(ref)) {
        if (elm.value !== ref.value)
            elm.value = ref.value;
        const text = elm.firstChild;
        if (text && isText(text) && text.textContent !== ref.value)
...
function morphChildNodes(element, ref, idMap) {
    ...
    for (let i = 0; i < refChildNodes.length; i++) {
        ...
        if (child && refChild)
            morphChildNode(child, refChild, element, idMap);
        else if (refChild)
...
function morphChildNode(child, ref, parent, idMap) {
    if (isElement(child) && isElement(ref))
        morphChildElement(child, ref, parent, idMap);
    else
        morphNodes(child, ref, idMap);
}
