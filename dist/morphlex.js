...
function morphNode(node, ref, context) {
    ...
    if (isElement(node) && isElement(ref) && node.tagName === ref.tagName) {
        ...
        if (isHead(node) && isHead(ref)) {
            ...
            for (const refChild of refChildNodes.values())
                appendChild(node, refChild.cloneNode(true), context);
        }
        else if (node.hasChildNodes() || ref.hasChildNodes())
            morphChildNodes(node, ref, context);
    }
    else {
        if (isText(node) && isText(ref)) {
            ...
            replaceNode(node, ref.cloneNode(true), context);
    }
...
function morphChildNodes(element, ref, context) {
    ...
    for (let i = 0; i < refChildNodes.length; i++) {
        ...
        if (child && refChild)
            morphChildNode(child, refChild, element, context);
        else if (refChild) {
...
function morphChildNode(child, ref, parent, context) {
    if (isElement(child) && isElement(ref))
        morphChildElement(child, ref, parent, context);
    else
        morphNode(child, ref, context);
}
function morphChildElement(child, ref, parent, context) {
    ...
    if (nextMatchByTagName) {
        ...
        replaceNode(child, ref.cloneNode(true), context);
}
